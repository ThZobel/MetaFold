...
window.importTemplatesFromFile = async function () {
    ...
    try {
        ...
        // Use Electron API to import templates - NEW: Support multiple selection
        const result = await window.electronAPI.importTemplatesFromFile('multi');
            ...
            for (const template of result.templates) {
                try {
                    ...
                    const cleanTemplate = {
                        ...
                        originalGroup: template.createdByGroup,
...
const enhancedActions = {
    ...
    openMoreMenu() {
        ...
        // Update content before opening, but don't let errors stop the menu from opening
        try {
            this.updatePanelContent();
        } catch (error) {
            console.error('⚠️ Error updating panel content (ignoring to allow menu open):', error);
        }
        ...
        if (overlay && panel) {
            ...
            // Fallback attempt: Try to find elements by class if IDs fail
            const panelByClass = document.querySelector('.slide-out-panel');
            const overlayByClass = document.querySelector('.slide-out-overlay');

            if (panelByClass && overlayByClass) {
                console.log('⚠️ Found elements by class, attempting fallback open...');
                overlayByClass.classList.add('active');
                panelByClass.classList.add('active');
                this.isOpen = true;
                document.body.style.overflow = 'hidden';
            }
        }
