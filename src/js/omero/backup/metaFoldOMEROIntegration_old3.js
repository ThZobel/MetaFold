...
const metaFoldOMEROIntegration = {
    ...
    hybridAuth: {
        ...
        // Dataset-Erstellung mit Gruppenzuordnung (KORRIGIERT)
        async createDatasetInGroup(name, description, groupId, projectId = null) {
            ...
            try {
                // KORRIGIERT: Kein Workaround mehr - verwende die gewünschte Gruppe direkt
                const targetGroupId = groupId;
                console.log('🔬 Verwende Zielgruppe:', targetGroupId);
                ...
                const saveUrl = `${this.proxyUrl}/api/v0/m/save/?group=${targetGroupId}`;
                ...
                if (result && result.data) {
                    ...
                    if (projectId && projectId !== 'none') {
                        await this.linkDatasetToProject(datasetId, projectId, targetGroupId);
                    }
    ...
    // Test der kompletten Integration (KORRIGIERT)
    async testCompleteIntegratedIntegration() {
        console.log('🧪 === TESTING COMPLETE INTEGRATED INTEGRATION (KORRIGIERT) ===');
        ...
        try {
            ...
            if (testResults.authentication) {
                ...
                if (groupInfo && groupInfo.memberOfGroups.length > 0) {
                    ...
                    console.log('📋 Group names mapping: 904="Public Example Data", others=see OMERO.web');
                } else {
                ...
                if (testResults.groupAccess) {
                    console.log('🧪 Test 3: Dataset Creation (using available group)...');
                    const testMetadata = {
                        test_field: {
                            type: 'text',
                            label: 'Test Field',
                            value: 'Integrated Hybrid Test - Corrected Version',
                    ...
                    const groupName = testGroupId === 904 ? 'Public Example Data' : `Group ${testGroupId}`;
                    
                    console.log(`🔬 Testing with group ${testGroupId} (${groupName})`);
                    
                    try {
                        const integrationResult = await this.createDatasetForMetaFoldProject(
                            `MetaFold_CorrectedTest_${Date.now()}`,
                            testMetadata,
                            {
                                groupId: testGroupId,
                                namespace: 'NFDI4BioImage.MetaFold.CorrectedTest'
                            }
                        );
            ...
            if (testResults.overall) {
                console.log('🎉 CORRECTED INTEGRATED SOLUTION IS READY FOR PRODUCTION!');
                console.log('💡 Group 904 = "Public Example Data" - no workaround needed');
            }
...
console.log("✅ MetaFold OMERO Integration - Korrigierte Lösung geladen!");
console.log("");
...
console.log("  Gruppe 904 und 'Public Example Data' sind identisch - kein Workaround nötig");
console.log("  Alle Gruppen werden jetzt korrekt behandelt");