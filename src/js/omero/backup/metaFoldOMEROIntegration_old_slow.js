...
const metaFoldOMEROIntegration = {
    ...
    hybridAuth: {
        ...
        // Projekt-Verknüpfung verifizieren
        async verifyProjectLinking(datasetId, projectId) {
            try {
                console.log('🔍 Verifiziere Projekt-Verknüpfung...');
                ...
                if (response.ok) {
                    ...
                    const linkedProjects = data.data?.parents || [];
                    
                    const isLinked = linkedProjects.some(p => 
                        (p.id || p['@id']) == projectId
                    );
                    
                    console.log('🔍 Projekt-Verknüpfung Status:', {
                        datasetId: datasetId,
                        targetProjectId: projectId,
                        linkedProjects: linkedProjects.map(p => ({ id: p.id || p['@id'], name: p.Name || p.name })),
                        isLinked: isLinked
                    });
                ...
                console.warn('⚠️ Projekt-Verknüpfung-Verifikation fehlgeschlagen:', error);
            }
    ...
    async createDatasetInGroup(name, description, groupId, projectId = null) {
        console.log('🔬 === KORREKTE DATASET-ERSTELLUNG IM PROJEKT ===');
        ...
        try {
            // Dataset-Daten (einfache Struktur)
            const datasetData = {
                "Name": name,
            ...
            let saveUrl, logMessage;
            
            if (projectId && projectId !== 'none') {
                // OPTION A: Dataset DIREKT IM PROJEKT erstellen (OMERO JSON API)
                saveUrl = `${this.proxyUrl}/api/v0/m/projects/${projectId}/datasets/?group=${groupId}`;
                logMessage = `Dataset DIREKT im Projekt ${projectId}`;
            } else {
                // OPTION B: Dataset standalone erstellen
                saveUrl = `${this.proxyUrl}/api/v0/m/save/?group=${groupId}`;
                logMessage = 'Dataset standalone';
            }
            
            console.log('🔬 Erstelle', logMessage);
            console.log('🔬 URL:', saveUrl);
            console.log('🔬 Dataset-Daten:', JSON.stringify(datasetData, null, 2));
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error('❌ Dataset-Erstellung fehlgeschlagen:', response.status, errorText);
                
                // Fallback: Wenn Projekt-Erstellung fehlschlägt, versuche standalone
                if (projectId && projectId !== 'none') {
                    console.log('🔄 Fallback: Versuche standalone Dataset-Erstellung...');
                    return await this.createDatasetInGroup(name, description, groupId, null);
                }
            ...
            // ROBUSTE ID-Extraktion (mehrere Versuche)
            let datasetId = null;
            
            // Versuch 1: Direkter Zugriff result.data['@id'] (funktionierende Version)
            if (result && result.data && result.data['@id']) {
                datasetId = result.data['@id'];
                console.log('🔍 ID gefunden (direkt):', datasetId);
            }
            
            // Versuch 2: Array-Zugriff result.data[0]['@id']
            if (!datasetId && result && result.data && Array.isArray(result.data) && result.data[0]) {
                datasetId = result.data[0]['@id'] || result.data[0].id;
                console.log('🔍 ID gefunden (Array):', datasetId);
            }
            
            // Versuch 3: Root-Level result['@id']
            if (!datasetId && result['@id']) {
                datasetId = result['@id'];
                console.log('🔍 ID gefunden (Root):', datasetId);
            }
            
            // Versuch 4: Vollständige Suche
            if (!datasetId) {
                const findId = (obj) => {
                ...
                datasetId = findId(result);
                console.log('🔍 ID gefunden (Suche):', datasetId);
            }
            
            if (datasetId) {
                console.log('✅ Dataset erfolgreich erstellt!');
                console.log('📋 Dataset-ID:', datasetId);
                console.log('📋 Erstellt in:', projectId ? `Projekt ${projectId}` : 'Standalone');
                ...
                // Projekt-Zuordnung verifizieren (falls im Projekt erstellt)
                let projectVerification = null;
                if (projectId) {
                    projectVerification = await this.verifyDatasetInProject(datasetId, projectId);
                    console.log('🔍 Projekt-Verifikation:', projectVerification);
                }
                
                return {
                    success: true,
                    datasetId: datasetId,
                    datasetName: result.data?.Name || name,
                    groupId: groupId,
                    projectId: projectId,
                    method: projectId ? 'Direct Project Creation' : 'Standalone Creation',
                ...
                throw new Error('Dataset-ID konnte nicht aus der Response extrahiert werden');
            }
    ...
    // NEUE FUNKTION: Verifizierung ob Dataset im Projekt ist
    async verifyDatasetInProject(datasetId, projectId) {
        try {
            console.log('🔍 Verifiziere Dataset-Projekt-Zuordnung...');
            
            // Projekt-Details abrufen und nach Dataset suchen
            const response = await fetch(`${this.proxyUrl}/api/v0/m/projects/${projectId}/`, {
                method: 'GET',
                credentials: 'include',
                headers: {
                    'Accept': 'application/json',
            ...
            if (response.ok) {
                ...
                // In Projekt-Datasets suchen
                const datasetsUrl = projectData.data?.['url:datasets'];
                if (datasetsUrl) {
                    const datasetsResponse = await fetch(`${this.proxyUrl}${datasetsUrl}`, {
                        method: 'GET',
                        credentials: 'include',
                        headers: {
                            'Accept': 'application/json',
                    ...
                    if (datasetsResponse.ok) {
                        ...
                        const isInProject = datasets.some(dataset => 
                            (dataset['@id'] || dataset.id) == datasetId
                        );
                        
                        console.log('🔍 Dataset im Projekt gefunden:', isInProject);
                        console.log('📋 Projekt-Datasets:', datasets.length);
                        
                        return {
                            success: isInProject,
                            isInProject: isInProject,
                            totalDatasets: datasets.length,
                            verification: 'project_datasets_check'
                        };
                    }
                }
            }
            ...
            return {
                success: false,
                message: 'Projekt-Verifikation nicht möglich',
                verification: 'failed'
            };
            
        } catch (error) {
            console.warn('⚠️ Projekt-Verifikation fehlgeschlagen:', error);
            return {
                success: false,
    ...
    // NEUE FUNKTION: Dataset mit direkter Projekt-Referenz erstellen
    async createDatasetWithDirectProjectLink(name, description, groupId, projectId) {
        console.log('🎯 === OMERO JSON API DATASET MIT PROJEKT v3.0 ===');
        console.log('🔗 Parent Project ID:', projectId);
        
        try {
            // Dataset-Daten mit expliziter parent-Referenz
            const datasetData = {
                "Name": name,
                "Description": description || `Erstellt von MetaFold für Projekt ${projectId}`,
                "@type": "http://www.openmicroscopy.org/Schemas/OME/2016-06#Dataset",
                "omero:details": {
                    "group": {
                        "@id": parseInt(groupId)
                ...
                // EXPLIZITE PROJEKT-REFERENZ (OMERO JSON API)
                "projectDatasetLinks": [{
            ...
            // JSON API Save-Endpoint mit Gruppen-Kontext
            const saveUrl = `${this.proxyUrl}/api/v0/m/save/?group=${groupId}`;
            
            console.log('🚀 Sende Dataset mit Projekt-Referenz an:', saveUrl);
            console.log('📋 Dataset-Daten:', JSON.stringify(datasetData, null, 2));
            ...
            if (response.ok) {
                ...
                if (datasetId) {
                    console.log('✅ Dataset mit Projekt-Referenz erfolgreich erstellt!');
                    console.log('📋 Dataset-ID:', datasetId);
                    console.log('🔗 Projekt-ID:', projectId);
                    
                    // Verifikation der Verknüpfung
                    const verification = await this.verifyProjectLinking(datasetId, projectId);
                    ...
                    return {
                        ...
                        groupId: groupId,
                        projectId: projectId,
                        method: 'direct_project_creation',
            ...
            console.log('❌ Dataset mit Projekt-Referenz fehlgeschlagen - HTTP', response.status);
            return {
                success: false,
                message: `HTTP ${response.status} - Direkte Projekt-Zuordnung nicht unterstützt`,
                method: 'direct_project_creation_failed'
            };
            
        } catch (error) {
            console.log('❌ Dataset mit Projekt-Referenz Exception:', error.message);
            return {
                success: false,
                message: error.message,
                method: 'direct_project_creation_exception'
            };
        }
    },

    // NEUE FUNKTION: Dataset standalone erstellen (ohne Projekt)

// KORRIGIERTE createDatasetStandalone Funktion
// Diese Funktion ersetzt die bestehende createDatasetStandalone Funktion im hybridAuth Objekt

    async createDatasetStandalone(name, description, groupId) {
        console.log('🏗️ === OMERO JSON API DATASET STANDALONE ===');
        
        try {
            // Einfache Dataset-Daten ohne Projekt-Referenz
            const datasetData = {
                "Name": name,
                "Description": description || 'Erstellt von MetaFold über robuste Hybrid Auth',
                "@type": "http://www.openmicroscopy.org/Schemas/OME/2016-06#Dataset"
            ...
            console.log('🚀 Sende standalone Dataset an:', saveUrl);
            ...
            // KORREKTUR: HTTP 201 und 200 sind beide Erfolg!
            if (response.ok || response.status === 201) {
                ...
                if (datasetId) {
                    console.log('✅ Standalone Dataset erfolgreich erstellt!');
                    console.log('📋 Dataset-ID:', datasetId);
                    
                    // Gruppen-Verifikation
                    const groupVerification = await this.verifyDatasetGroup(datasetId, groupId);
                    
                    return {
                        success: true,
                        datasetId: datasetId,
                        groupId: groupId,
                        method: 'standalone_creation',
                    ...
                    // Erfolgreiche Response, aber keine Dataset-ID gefunden
                    console.warn('⚠️ HTTP', response.status, 'aber keine Dataset-ID in Response gefunden');
                    console.log('📋 Full response:', result);
                    
                    return {
                        success: false,
                        message: `Dataset erstellt (HTTP ${response.status}), aber ID nicht gefunden`,
                        method: 'standalone_creation_no_id',
                        httpStatus: response.status,
                        rawResponse: result
                    };
                }
            } else {
                // Tatsächlicher HTTP Fehler
                const errorText = await response.text();
            ...
            console.error('❌ Standalone Dataset-Erstellung Exception:', error);
            return {
                success: false,
                message: error.message,
                method: 'standalone_creation_exception'
            };
        }
    },

    // NEUE FUNKTION: Projekt-Verknüpfung verifizieren
    async verifyProjectLinking(datasetId, projectId) {
        try {
            console.log('🔍 Verifiziere Projekt-Verknüpfung...');
            console.log('📋 Dataset-ID:', datasetId);
            ...
            // Dataset-Details über JSON API abrufen
            const response = await fetch(`${this.proxyUrl}/api/v0/m/datasets/${datasetId}/`, {
                method: 'GET',
                credentials: 'include',
                headers: {
            ...
            if (response.ok) {
                const data = await response.json();
                const projectLinks = data.data?.projectDatasetLinks || [];
                
                const isLinked = projectLinks.some(link => 
                    link.parent && link.parent['@id'] == projectId
                );
                
                console.log('🔍 Projekt-Verknüpfung gefunden:', isLinked);
                console.log('📋 Verknüpfte Projekte:', projectLinks.map(link => link.parent?.['@id']));
                
                return {
                    success: true,
                    isLinked: isLinked,
                    linkedProjects: projectLinks.map(link => link.parent?.['@id']),
                    verification: 'json_api'
                };
            }
            
            return {
                success: false,
                message: 'Verifikation über JSON API fehlgeschlagen',
                verification: 'failed'
            };
            
        } catch (error) {
            console.warn('⚠️ Projekt-Verknüpfungs-Verifikation fehlgeschlagen:', error);
            return {
                success: false,
                message: error.message,
                verification: 'exception'
        ...
        // Dataset-Projekt-Verknüpfung (VERBESSERTE VERSION)
        async linkDatasetToProject(datasetId, projectId, groupId) {
            console.log('🔬 === DATASET-PROJEKT-VERKNÜPFUNG ===');
            console.log('🔬 Dataset-ID:', datasetId);
            console.log('🔬 Projekt-ID:', projectId);
            console.log('🔬 Gruppen-ID:', groupId);
            
            try {
                // Methode 1: JSON API verwenden (bevorzugt)
                console.log('🔬 Versuche JSON API Projekt-Verknüpfung...');
                ...
                const jsonApiUrl = `${this.proxyUrl}/api/v0/m/projects/${projectId}/datasets/`;
                ...
                if (jsonResponse.ok) {
                    ...
                    console.log('✅ Dataset erfolgreich mit Projekt über JSON API verknüpft');
                    console.log('📋 Antwort:', result);
                    return { success: true, method: 'JSON API', response: result };
                } else {
                    console.warn('⚠️ JSON API Verknüpfung fehlgeschlagen:', jsonResponse.status, jsonResponse.statusText);
                    
                    // Methode 2: Webclient addnewcontainer (Fallback)
                    console.log('🔬 Versuche Webclient Projekt-Verknüpfung...');
                    ...
                    console.log('🔬 Sende Webclient-Verknüpfung an:', linkUrl);
                    ...
                    const webclientResponse = await fetch(linkUrl, {
                        method: 'POST',
                        credentials: 'include',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                    ...
                    if (webclientResponse.ok) {
                        console.log('✅ Dataset erfolgreich mit Projekt über Webclient verknüpft');
                        return { success: true, method: 'Webclient', response: await webclientResponse.text() };
                    } else {
                        const errorText = await webclientResponse.text();
                        console.error('❌ Webclient Verknüpfung fehlgeschlagen:', webclientResponse.status, errorText);
                        throw new Error(`Projekt-Verknüpfung fehlgeschlagen: ${webclientResponse.status} - ${errorText}`);
                    }
                }
                
            } catch (error) {
                console.error('❌ Dataset-Projekt-Verknüpfung komplett fehlgeschlagen:', error);
                
                // NICHT werfen - Dataset wurde erfolgreich erstellt, Verknüpfung ist optional
                console.warn('⚠️ Dataset wurde erstellt, aber Projekt-Verknüpfung fehlgeschlagen');
                console.warn('⚠️ Bitte verknüpfe Dataset', datasetId, 'manuell mit Projekt', projectId, 'in OMERO.web');
                
                return { 
                    success: false, 
                    message: `Projekt-Verknüpfung fehlgeschlagen: ${error.message}`,
                    recommendation: `Verknüpfe Dataset ${datasetId} manuell mit Projekt ${projectId} in OMERO.web`
                };
            }
        },
...
// Einfacher Test-Befehl für Projekt-Verknüpfung (ähnlich dem funktionierenden Code)
window.testOMEROProjectLinking = async function(datasetId, projectId) {
    try {
        console.log('🚀 Teste OMERO Projekt-Verknüpfung...');
        console.log('📋 Dataset-ID:', datasetId);
        console.log('📋 Projekt-ID:', projectId);
        
        if (!metaFoldOMEROIntegration.hybridAuth.session) {
            console.log('❌ Keine aktive OMERO Session - führe Login durch...');
            await metaFoldOMEROIntegration.ensureIntegratedAuthentication();
        }
        
        // Teste Projekt-Verknüpfung
        const linkResult = await metaFoldOMEROIntegration.hybridAuth.linkDatasetToProject(
            datasetId, 
            projectId, 
            metaFoldOMEROIntegration.hybridAuth.session.groupId
        );
        
        console.log('🔗 Verknüpfungs-Ergebnis:', linkResult);
        
        // Verifiziere Verknüpfung
        const verification = await metaFoldOMEROIntegration.hybridAuth.verifyProjectLinking(datasetId, projectId);
        console.log('🔍 Verknüpfungs-Verifikation:', verification);
        
        if (linkResult.success && verification.isLinked) {
            console.log('✅ Projekt-Verknüpfung erfolgreich!');
            return { success: true, linkResult, verification };
        } else {
            console.log('❌ Projekt-Verknüpfung fehlgeschlagen');
            return { success: false, linkResult, verification };
        }
        
    } catch (error) {
        console.error('❌ Projekt-Verknüpfungs-Test fehlgeschlagen:', error);
...
console.log("✅ MetaFold OMERO Integration - KORRIGIERTE VERSION v2.0 mit expliziter parent-Referenz!");
console.log("");
...
console.log("  await testOMEROProjectLinking(datasetId, projectId)          // Projekt-Verknüpfung testen");
console.log("");
console.log("💡 VERSION v2.0 ÄNDERUNGEN:");
...
console.log("  ✅ EXPLIZITE OMERO JSON API: Dataset mit parent-Referenz erstellen");
console.log("  ✅ Detailliertes Logging für Debugging");
console.log("  ✅ Projekt-Verknüpfungs-Verifikation mit Fallback");
console.log("  ✅ Debug-Informationen für Troubleshooting");
console.log("  ✅ Korrekte settingsManager.get(key) API");
console.log("");
console.log("📚 BASED ON: https://omero.readthedocs.io/en/stable/developers/json-api.html");
console.log("");
console.log("🔍 VERSION CHECK: Schaue nach '=== OMERO JSON API DATASET-ERSTELLUNG v2 ===' im Log!");