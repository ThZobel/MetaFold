...
class OMEROProxyHandler(BaseHTTPRequestHandler):
    ...
    def process_response_cookies(self, response, client_id):
        """Process and store response cookies - FIXED for CSRF consistency"""
        ...
        if set_cookie_headers:
            ...
            if cookie_strings:
                with self.session_lock:
                    ...
                    # Smart cookie merging - PRESERVE CSRF token consistency
                    if existing_cookies:
                        existing_dict = {}
                        ...
                        for cookie in cookie_strings:
                            if '=' in cookie:
                                key, value = cookie.strip().split('=', 1)
                                # ✅ CRITICAL FIX: Only update if not CSRF token or if it's a new session
                                if key != 'csrftoken' or client_id not in self.client_sessions:
                                    existing_dict[key] = value
                                else:
                                    print(f"🔧 CSRF Fix: Preserving existing CSRF token for {client_id}")
    ...
    def send_successful_response(self, response, response_data):
        """Send successful response to client - FIXED CSRF cookie handling"""
        response_headers = response.info()
        ...
        # ✅ CRITICAL FIX: Forward cookies WITHOUT modification for CSRF tokens
        for header_name, header_value in response_headers.items():
            if header_name.lower() == 'set-cookie':
                # Check if this is a CSRF token cookie
                if 'csrftoken=' in header_value:
                    # Forward CSRF token cookies UNCHANGED
                    print(f"🔧 CSRF Fix: Forwarding CSRF cookie unchanged: {header_value[:50]}...")
                    self.send_header('Set-Cookie', header_value)
                else:
                    # Apply normal modifications to other cookies
